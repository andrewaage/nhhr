...
4. Convert y to a character-vector. 
...
Run use a summary on the dataframe called student_df:
```{r dataframe_summary, exercise=TRUE}

```

In order to illustrate the usefulness of putting data in a dataframe, let's look at the relationship between age and grade. For now, just run the code below - we will come back to plotting later on. 
...
In the same density plot, choose a different *fill* for the different groups of creditability. Consider using the alpha-parameter to make the plots easier to see. NB! You might have to convert Creditability to a factor first.
...
ggplot(credit_df, aes(x = Age, fill = factor(Creditability))) +
...
Illustrate the relationship between Creditability and Occupation. Choose a suitable *geom* (this one is a bit harder...)
...
Create the same plot, except by coloring the different groups of Creditability in different colors. 

```{r scattercolor, exercise = TRUE}

```

Create the same plot, but now add a third variable: split the plot by occupation. Use facet_wrap. 
